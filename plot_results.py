...
rects1 = ax1.bar(x - width/2, accs, width, label='Average Accuracy (%)', color='#2ca02c')
rects2 = ax1.bar(x + width/2, forgets, width, label='Forgetting (%)', color='#d62728')

ax1.set_ylabel('Percentage (%)', fontsize=12)
ax1.set_title('Continual Learning Performance (Split-MNIST, 5 Tasks)', fontsize=14, pad=20)
ax1.set_xticks(x)
...
colors = {'Naive Fine-tuning': '#7f7f7f', 'Experience Replay (Buffer=250)': '#1f77b4', 
          'PAL-MoE (Ours)': '#ff7f0e', 'PAL-MoE + Replay (Hybrid, P=250)': '#2ca02c'}
...
for m in methods:
    ...
    ax2.plot(range(1, 6), evolution, marker=marker, linewidth=2.5, markersize=8, 
