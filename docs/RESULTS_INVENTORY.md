...
| `ac3/ac3_address_space_study.json` | address-space ablation (fixed retrieval vs learned address), 6 re-scored + 6 reference cells, all guards exact | V2 | AC3_ADDRESS_SPACE_RESULTS.md |
...
path stays invariant. AC3 shows the address does not have to be learned at all: a
parameter-free prototype retrieval attains E0's coverage inside the E2 value path at
equal accuracy (Delta Acc = +0.048 pp, equivalent within +/-1 pp), so the v2
invariant is **stable keys / plastic reader**, and the open variable is the value
path (AC2).

## Conventions for local artifacts

`results/` is **not tracked** (`.gitignore`): the study JSONs, caches and logs are
local research artifacts, regenerated by the runners under `experiments/`, and this
inventory references them by path. Each study JSON records the harness revision it
was run from; studies written before the 2026-09-25 history rewrite carry
pre-rewrite hashes, and the full old-to-new commit map is kept outside the repo next
to the history bundle.
