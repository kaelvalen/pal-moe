...
frozen base (immutable, content-hashed)
 ...
 O(1) write, exact delete       canonical order, subtractable    (by_arrival | by_confusion*)
...
are additive over edits. Contributions are kept per edit and summed in a **canonical
order** (content hash, then id), so the solution is bitwise independent of arrival
order and `forget` re-solves from the remaining set - bitwise equal to never having
written the edit. The arrival-order running sum is kept only to report drift
(guard 3). On the LM the same structure is the MEMIT closed form with a key-covariance
prior, solved through Woodbury (`edit/down_proj.py`), with residuals computed against
the **base** model so contributions stay independent.
...
| `forget(edit_id)` | a report: whether the resulting state was seen before, and whether weights and canary outputs match it bitwise |
...
| 2 | **Reversibility** | inside every write a trial undo/redo: undo must restore the weights digest and canary outputs bitwise, redo the post-write outputs; on every forget, the resulting state's digests must equal those recorded when that state was last visited | roll back, raise |
| 3 | **Order invariance** | canonical solution vs the arrival-order running sum: canary argmax identical, max \|dW\| reported | roll back, raise |
...
pal_moe/
  ...
  edit/        LinearStats (float64, canonical, subtractable); DownProjEdit (LM)
...
| float64 statistics, canonical summation order | float32 continual vs one-shot max\|dW\| 1.26e-3 / 1.09e-3 (G3) | `E_TID2_RESULTS.md` |
...
| E-TID2 cells, all five arms + coverage + G1/G2 | 12 | 1e-6 | 0.0 | 0.0 | 0.0 on all five arms and both coverages, ridge arms included (float64 moved no argmax); G3 max\|dW\| 3.6e-13 (float32 run: 1.26e-3); all writes and the consolidation reversible bitwise |
...
| editing harness (CounterFact / zsRE / MQuAKE / canary) | built, tested on synthetic records (`tests/test_v3_eval.py`); the study is `V3_LLM_PREREG.md`, **not run** |
