...
def needs(p):
    return pytest.mark.skipif(
        not p.exists(), reason=f"{p.name} not present (results/ is untracked)"
    )
...
def test_moved_names_are_the_same_objects():
    ...
    assert (
        s2_ladder.load_tasks is feats.load_tasks
        and s2_ladder.set_seed is feats.set_seed
    )
...
def test_anchor_ac3_fixed_proto_cell():
    ...
    ref = next(
        c
        for c in json.loads(AC3_JSON.read_text())["cells"]
        if c["regime"] == "coherent" and c["seed"] == 42
    )
    got = ac3.run_cell(
        "coherent",
        42,
        argparse.Namespace(epochs=10, lr=1e-3, batch_size=128),
        torch.device(DEVICE),
    )
...
def test_anchor_etid2_cell(api):
    from v3_anchors import ETID2_ARMS, ETID2_COV, check_etid2

    rows = check_etid2([42], None, torch.device(DEVICE), api=api)
    bank = ("proto", "oracle", "proto_C@1", "proto_C@3")
    for r in rows:
        fields = bank if api else ETID2_ARMS + ETID2_COV
        assert max(abs(r[f"delta_{k}"]) for k in fields) <= 1e-6
