#!/usr/bin/env bash
# Paper wave 1: consolidation runs from EXPERIMENT_PLAN.md.
#
# Order: E1 (ViT promotion) -> E2 (CIFAR-100 ResNet-18 3-seed) ->
#        E4 (equal-byte Pareto) -> E7 (growth/reuse) -> E5 (component ablation)
#        -> E8 (anchor drift) -> E9 (capacity/cost) -> E3 (headline regen, fast)
#
# Every step logs to results/paper_wave1.log and a failure does not stop the
# queue (so one bad cell cannot cost the whole night). Idempotent-ish: re-running
# a step overwrites its output dir.
set -u
cd "$(dirname "$0")/../.."
export PYTHONPATH=.
...
# ---------------------------------------------------------------- E1a
step "E1a CIFAR-10 ViT-B/16, seeds 42 1 2"
$PY experiments/run_benchmark_multi.py --seeds "42 1 2" --device cuda \
  --config configs/cifar10_vit.json \
  --output_dir results/cifar10_vit_multiseed || echo "FAILED E1a"

# ---------------------------------------------------------------- E2
step "E2 CIFAR-100 ResNet-18, seeds 42 1 2"
bash experiments/recipes/cifar100_resnet18_multiseed.sh || echo "FAILED E2"

# ---------------------------------------------------------------- E1b
step "E1b CIFAR-100 ViT-B/16, seeds 42 1 2 (builds the feature cache first)"
$PY experiments/run_benchmark_multi.py --seeds "42 1 2" --device cuda \
  --config configs/cifar100_vit.json \
  --output_dir results/cifar100_vit_multiseed || echo "FAILED E1b"

# ---------------------------------------------------------------- E4a
# Equal-byte Pareto, CIFAR-10 ResNet-18. Item counts are derived from the byte
# target: raw image = 3072*4 + 8 label bytes = 12296; DER++ adds C*4 logits;
# iCaRL stores k per class; PAL pure ~2184 B/prototype; hybrid ~14400 B/prototype
# (latent + one raw image). The realised bytes are in memory_bytes in each JSON.
...
step "E4a equal-byte CIFAR-10 ResNet-18 (seed 42, four budgets)"
for BUDGET in 262144 1048576 4194304 16777216; do
  B=$((BUDGET / 12296)); D=$((BUDGET / 12336)); K=$((BUDGET / 122960))
  P=$((BUDGET / 2184)); H=$((BUDGET / 14400))
  PS=$((P / 5 * 2)); [ "$PS" -lt 256 ] && PS=256; [ "$PS" -gt 5000 ] && PS=5000
  echo "--- budget ${BUDGET} B: replay=${B} derpp=${D} icarl_k=${K} pure_proto=${P} hybrid_proto=${H} proto_samples=${PS}"
  ...
  run_c10 42 "$BUDGET" hybrid --proto_size "$H" --proto_samples "$PS"
done
step "E4a equal-byte CIFAR-10 ResNet-18 (seeds 1 2, 1 MiB and 4 MiB)"
for SEED in 1 2; do
  for BUDGET in 1048576 4194304; do
    ...
    run_c10 "$SEED" "$BUDGET" hybrid --proto_size "$H" --proto_samples "$PS"
  done
done

# ---------------------------------------------------------------- E4b
# Equal-byte Pareto, CIFAR-100 ResNet-18. Raw item 12296 B; DER++ +400 B logits;
# iCaRL 1.23 MB per k (infeasible below ~1.2 MB); PAL pure ~2476 B/prototype;
# hybrid ~14764 B/prototype. Budgets 256 KiB / 1 MiB / 4 MiB.
...
step "E4b equal-byte CIFAR-100 ResNet-18 (seed 42, three budgets)"
for BUDGET in 262144 1048576 4194304; do
  B=$((BUDGET / 12296)); D=$((BUDGET / 12696)); K=$((BUDGET / 1229600))
  P=$((BUDGET / 2476)); H=$((BUDGET / 14764))
  ...
  run_c100 42 "$BUDGET" hybrid --proto_size "$H" --proto_samples "$PS"
done
step "E4b equal-byte CIFAR-100 ResNet-18 (seeds 1 2, 1 MiB)"
for SEED in 1 2; do
  BUDGET=1048576
  B=$((BUDGET / 12296)); D=$((BUDGET / 12696)); K=$((BUDGET / 1229600))
  P=$((BUDGET / 2476)); H=$((BUDGET / 14764))
  ...
  run_c100 "$SEED" "$BUDGET" hybrid --proto_size "$H" --proto_samples "$PS"
done

# ---------------------------------------------------------------- E7
# Growth/reuse: gated allocation vs forced one-expert-per-task, with the
# per-task expert counts and routing retention recorded. CIFAR-100 ResNet-18.
...
# ---------------------------------------------------------------- E5
# Component ablation at the final recipe (CIFAR-10 ResNet-18, 3 seeds).
# Variant 4/5 are the "static" ladder: forced expansion, mechanisms off.
...
# ---------------------------------------------------------------- E8
# Anchor refresh / stale-anchor study, CIFAR-100 ResNet-18, 3 seeds.
...
run_drift refresh_off_alpha "$SEED" --proto_routing_alpha 0.5
run_drift refresh_on_alpha "$SEED" --refresh_anchors_after_calib --proto_routing_alpha 0.5

# ---------------------------------------------------------------- E9
# Capacity scaling + parameter-matched baseline, CIFAR-100 ResNet-18, 3 seeds.
...
for SEED in 42 1 2; do
  ...
  # Parameter-matched single head: hidden 4600 gives ~1.65M params, matching the
  # 6-expert PAL total on this benchmark (active params stay comparable).
  $PY experiments/run_benchmark.py --config configs/cifar100_resnet18_frozen.json \
...
step "PAPER_WAVE1_DONE $(date '+%F %T')"
