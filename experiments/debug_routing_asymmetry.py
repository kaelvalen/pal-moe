...
def inspect_routing_distribution(model: DynamicMoE, tasks, device: torch.device):
    ...
    for t_idx, task in enumerate(tasks):
        ...
        with torch.no_grad():
            ...
            top1_fractions = [(top1_choice == i).float().mean().item() for i in range(num_experts)]
...
def run_diagnostic_experiment(
    ...
    print(f"\n{'='*70}")
    print(f"DIAGNOSTIC RUN: Encoder={encoder_type.upper()} ({pretrain_epochs} ep), Seed={seed}")
    print(f"{'='*70}")
    set_seed(seed)

    tasks = get_split_mnist_tasks(data_dir="./data", batch_size=128, val_split=0.1, seed=seed)
    ...
    if encoder_type == "contrastive":
        ...
        base_encoder.pretrain_contrastive(unlabeled_loader, device=device, epochs=pretrain_epochs, lr=1e-3)
    else:
        print("  Running standard Autoencoder reconstruction pretraining...")
        base_encoder.pretrain_unsupervised(unlabeled_loader, device=device, epochs=pretrain_epochs)
    base_encoder.freeze()
    ...
    for t_idx, task in enumerate(tasks):
        ...
        for s in stats[:t_idx + 1]:
            routes = ", ".join([f"E{i}:{f:.0%}" for i, f in enumerate(s["top1_fractions"])])
        ...
        print(tabulate(table_rows, headers=["Task", "MoE Acc", "Top-1 Routing Choices", "Individual Expert Accs"], tablefmt="simple"))
...
def main():
    ...
    with open(summary_file, "w") as f:
        json.dump({"autoencoder_1ep": res_ae, "contrastive_simclr_3ep": res_simclr}, f, indent=2)
